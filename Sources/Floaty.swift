...
open class Floaty: UIView {
    ...
    static var allFloatyInstances = [Floaty]()
     ...
     */
    @objc open var paddingX: CGFloat = 5 {
        didSet {
            self.setNeedsDisplay()
        }
    }
    @objc open var paddingY: CGFloat = 5 {
        didSet {
            self.setNeedsDisplay()
        }
    }
     ...
     */
    @IBInspectable
    @objc open var buttonColor: UIColor = UIColor(red: 48/255.0, green: 63/255.0, blue: 159/255.0, alpha: 1)
     ...
     */
    @IBInspectable
    @objc open var itemSpace: CGFloat = 5
    ...
    public override init(frame: CGRect) {
        ...
        backgroundColor = UIColor.clear
        Floaty.allFloatyInstances.append(self)
        isCustomFrame = true
        setObserver()
        setAccessibilityView()
    }
    ...
    func closeOtherFloatyInstances() {
        for floaty in Floaty.allFloatyInstances {
            if floaty !== self {
                floaty.close()
            }
        }
     ...
     */
    @discardableResult
    open func addItem(title: String, titlePosition: FloatyItemLabelPositionType = .left) -> FloatyItem {
        let item = FloatyItem()
        itemDefaultSet(item)
     ...
     */
    @discardableResult
    open func addItem(_ title: String, icon: UIImage?, titlePosition: FloatyItemLabelPositionType = .left) -> FloatyItem {
        let item = FloatyItem()
        itemDefaultSet(item)
        item.titleLabelPosition = titlePosition
        item.title = title
     ...
     */
    @discardableResult
    open func addItem(title: String, titlePosition: FloatyItemLabelPositionType = .left, handler: @escaping ((FloatyItem) -> Void)) -> FloatyItem {
        let item = FloatyItem()
        itemDefaultSet(item)
        item.titleLabelPosition = titlePosition
        item.title = title
     ...
     */
    @discardableResult
    open func addItem(_ title: String, icon: UIImage?, titlePosition: FloatyItemLabelPositionType = .left, handler: @escaping ((FloatyItem) -> Void)) -> FloatyItem {
        let item = FloatyItem()
        itemDefaultSet(item)
        item.titleLabelPosition = titlePosition
        item.title = title
    ...
    fileprivate func determineTapArea(item : FloatyItem) -> CGRect {
        let tappableMargin : CGFloat = 5.0
        var x : CGFloat?
    ...
    deinit {
        ...
        if let index = Floaty.allFloatyInstances.firstIndex(of: self) {
            Floaty.allFloatyInstances.remove(at: index)
        }
    }
    ...
    fileprivate func popAnimationWithOpen(group: DispatchGroup) {
        ...
        for item in items {
            ...
            if verticalDirection == .up {
                ...
                item.frame.origin.y = itemHeight
            }else  if verticalDirection == .left {
...
// MARK: - mark - Accessibility Handling
extension Floaty {
